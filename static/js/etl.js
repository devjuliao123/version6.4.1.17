...
function parseObservacoes(observacoes) {
    ...
    try {
        const textoLimpo = observacoes.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
        
        const extracaoMatch = textoLimpo.match(/EXTRAÇÃO:?\s*\[(.*?)\]/i);
        if (extracaoMatch && extracaoMatch[1].trim() !== '') {
            resultado.extracao = extracaoMatch[1].split(',').map(item => item.trim()).filter(item => item !== '');
        }
        
        const historicoMatch = textoLimpo.match(/HISTÓRICO:?\s*\[(.*?)\]/i);
        if (historicoMatch && historicoMatch[1].trim() !== '') {
            resultado.historico = historicoMatch[1].split(',').map(item => item.trim()).filter(item => item !== '');
        }
        
        const diasMatch = textoLimpo.match(/DIAS:?\s*\[(\d+)\]/i);
        if (diasMatch) {
            resultado.dias = parseInt(diasMatch[1]) || 0;
        }
        
        const registrosMatch = textoLimpo.match(/REGISTROS:?\s*\[(.*?)\]/i);
        if (registrosMatch && registrosMatch[1].trim() !== '') {
            const pares = registrosMatch[1].split(',');
            
            pares.forEach(par => {
                const [tipo, valor] = par.split(':').map(s => s.trim());
                if (tipo && valor) {
                    const numero = parseInt(valor.replace(/\./g, '')) || 0;
                    ...
                    if (resultado.extracao.includes(tipo)) {
                        resultado.totalRegistrosExtracao += numero;
                    } else if (resultado.historico.includes(tipo)) {
                        resultado.totalRegistrosHistorico += numero;
                    }
                }
            });
        }
        
        const complexidadeMatch = textoLimpo.match(/COMPLEXIDADE:?\s*\[(.*?)\]/i);
...
function temDadosETL(infoETL) {
    ...
    return infoETL.totalRegistrosExtracao > 0 || infoETL.dias > 0;
}
...
function mostrarFiliaisPorComplexidade(complexidade) {
    ...
    const filiais = base.filter(item => {
        ...
        totalRegistros: parseObservacoes(item.observacoes).totalRegistros,
        dias: parseObservacoes(item.observacoes).dias
    }));
        ...
        <div>
            ...
            ">
                ${ordenados.map((f, index) => `
                    ...
                    ">
                        <div style="flex: 2;">
                            <div style="font-weight: 600; margin-bottom: 4px;">${f.filial}</div>
                            <div style="font-size: 0.8rem; color: var(--text-secondary);">${f.cliente}</div>
                            ${f.dias > 0 ? `<div style="font-size: 0.7rem; color: var(--text-secondary); margin-top: 4px;">⏱️ ${f.dias} dias</div>` : ''}
                        </div>
                        <div style="text-align: right;">
                            <div style="font-weight: 700; color: ${corAtual.cor};">${formatNumber(f.totalRegistros)}</div>
                            <div style="font-size: 0.7rem; color: var(--text-secondary);">registros</div>
                        </div>
                    </div>
                `).join('')}
            </div>
...
function mostrarFiliaisPorTipoExtracao(tipo) {
    ...
    const filiais = base.filter(item => {
        ...
        cliente: item.filial_descricao,
        tipo: tipo,
        totalRegistros: parseObservacoes(item.observacoes).totalRegistros || 0
    }));
